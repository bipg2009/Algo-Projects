...
def load_backtest_module():
    file_path = os.path.join(os.path.dirname(__file__), "NSE-Option-Scanner-Backtest-TEST.py")
...
def run_multiday(start_date, end_date, expiry_code=1):
    ...
    for mode in modes:
        ...
        for i, date_obj in enumerate(dates):
            ...
            print(f"\n[{i+1}/{len(dates)}] Scanning Date: {date_str} in {mode} mode...")
            ...
            try:
                ...
                print(f"Skipping date {date_str} in {mode} mode due to error: {e}")

        # --- PRINT SUMMARY FOR CURRENT MODE IMMEDIATELY ---

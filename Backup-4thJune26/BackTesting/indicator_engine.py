...
from System_Config import EMA_FAST, EMA_SLOW, VOLUME_EMA_PERIOD

# Module-level mutable state: tracks per-contract volume history across ticks
CONTRACT_VOLUME_HISTORY: dict = {}
...
def calculate_rsi_series(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    try:
        ...
        SafetyLogger.log_error_with_context(
            "indicator_engine", "calculate_rsi_series", e, {"period": period}
        )
...
def calculate_rsi(df: pd.DataFrame, period: int = 14) -> float:
    try:
        ...
        SafetyLogger.log_error_with_context(
            "indicator_engine", "calculate_rsi", e, {"period": period}
        )
        return 50.0
...
def add_ema(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        if (
            "EMA9" in df.columns
            and "EMA20" in df.columns
            and not pd.isna(df["EMA9"].iloc[-1])
        ):
            return df
        ...
        work["EMA50"] = close_series.ewm(span=50, adjust=False).mean()

        # MTF Equivalents: 5-minute trend calculated on a 1-minute chart (Multiplier = 5)
        work["EMA_5m_20"] = close_series.ewm(span=100, adjust=False).mean()  # 20 * 5
        work["EMA_5m_50"] = close_series.ewm(span=250, adjust=False).mean()  # 50 * 5

        # MTF Equivalents: 15-minute trend calculated on a 1-minute chart (Multiplier = 15)
        work["EMA_15m_20"] = close_series.ewm(span=300, adjust=False).mean()  # 20 * 15
        work["EMA_15m_50"] = close_series.ewm(span=750, adjust=False).mean()  # 50 * 15
        ...
        fallback["EMA50"] = fallback.get("close", 0.0)
        fallback["EMA_5m_20"] = fallback.get("close", 0.0)
        fallback["EMA_5m_50"] = fallback.get("close", 0.0)
        fallback["EMA_15m_20"] = fallback.get("close", 0.0)
        fallback["EMA_15m_50"] = fallback.get("close", 0.0)
        return fallback


def calculate_volume_ema(
    history: list, current_volume: float, period: int = 20
) -> float:
    try:
        if not history:
...
def parse_discrete_1m_volume(symbol: str, cumulative_volume: int) -> tuple:
    try:
        ...
        disc_vol = max(0, cumulative_volume - prev)
...
def add_supertrend(df: pd.DataFrame, period=10, multiplier=3) -> pd.DataFrame:
    """
    Optimized Supertrend implementation.

    Outputs:
        supertrend    -> Supertrend line
        ST_direction  -> 1 (Bullish) / -1 (Bearish)
        st_color      -> GREEN / RED (backward compatibility)
        ATR           -> ATR used by Supertrend
    """
    try:
        if (
            df is None
        ...
        required_cols = ["supertrend", "ST_direction", "st_color"]

        if all(col in df.columns for col in required_cols):
            if not pd.isna(df["supertrend"].iloc[-1]):
                return df
        ...
        # Keep ATR available to other modules
        df["ATR"] = atr
        ...
        st_line = np.zeros(length)
        st_dir = np.ones(length, dtype=int)

        # Proper initialization
        final_ub[0] = basic_ub.iloc[0]
        final_lb[0] = basic_lb.iloc[0]
        st_line[0] = basic_lb.iloc[0]

        c_vals = close.fillna(0.0).values

        for i in range(1, length):

            # Final Upper Band
            if basic_ub.iloc[i] < final_ub[i - 1] or c_vals[i - 1] > final_ub[i - 1]:
                final_ub[i] = basic_ub.iloc[i]
            else:
                final_ub[i] = final_ub[i - 1]

            # Final Lower Band
            if basic_lb.iloc[i] > final_lb[i - 1] or c_vals[i - 1] < final_lb[i - 1]:
            ...
            if st_line[i - 1] == final_ub[i - 1]:
                st_dir[i] = -1 if c_vals[i] <= final_ub[i] else 1
            else:
                st_dir[i] = 1 if c_vals[i] >= final_lb[i] else -1
        ...
        df["st_color"] = np.where(st_dir == 1, "GREEN", "RED")
        ...
        fallback["supertrend"] = (
            fallback["close"] if "close" in fallback.columns else 0.0
        )

        fallback["ST_direction"] = 1
        fallback["st_color"] = "GREEN"
...
def add_adx(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
    try:
        ...
        SafetyLogger.log_error_with_context("indicator_engine", "add_adx", e)
        df["ADX"] = 0.0
        df["ATR"] = 15.0  # Safe default fallback
        return df
...
def display_atr_details(tsl):
    ...
    try:
        ...
        if df is None or df.empty:
            print(
                "[!] Could not fetch historical data for NIFTY to calculate ATR.",
                flush=True,
            )
            return
        ...
        df["tr"] = pd.DataFrame(
            {"tr1": df["tr1"], "tr2": df["tr2"], "tr3": df["tr3"]}
        ).max(axis=1)
        ...
        direction = (
            "UP (Expanding Volatility)"
            if latest_atr > prev_atr
            else "DOWN (Contracting Volatility)"
        )
        ...
        SafetyLogger.log_error_with_context(
            "indicator_engine", "display_atr_details", e
        )

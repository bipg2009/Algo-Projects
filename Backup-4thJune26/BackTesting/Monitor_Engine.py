...
def _check_trailing_stop(
    ...
    # The requirement: "At 2 mins: If LTP < Buying Price +.50 Exit"
    # To prevent immediate exit on a later tick, we enforce this check only if SL hasn't been upgraded to Entry + 0.50 yet.
    cost_plus_50 = round(position.entry_ltp + 0.50, 2)
    if elapsed_min >= 2.0 and position.sl < cost_plus_50:
        if current_ltp < cost_plus_50:
            print(f"[{current_time}] EXIT: 2_min_profit_check_failed at LTP {current_ltp}", flush=True)
            return ("EXIT", "2_min_profit_check_failed", current_ltp)
        else:
            # Upgrade SL immediately to Cost + 0.50
            return ("TRAIL", "2_min_cold_protection_end", cost_plus_50)
    ...
    if elapsed_min >= 6.0 and position.sl < cost_plus_250:
        if current_ltp < cost_plus_250:
            print(f"[{current_time}] EXIT: 6_min_momentum_failed at LTP {current_ltp}", flush=True)
    ...
    phase_min_sl = cost_plus_250 if elapsed_min >= 6.0 else cost_plus_50

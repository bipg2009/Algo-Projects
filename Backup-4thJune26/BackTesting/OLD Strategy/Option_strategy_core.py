...
def detect_trigger_1m(*args, **kwargs):
    try:
        ...
        if option_type == "CE":
            nifty_dir_pass = (55 < nifty_rsi < 68) and (close_price > ema20) and (close_price > vwap_price)
            opt_exec_pass = (opt_rsi > 60) and opt_volume_spike
            if not (nifty_dir_pass and atr_expanding and opt_exec_pass):
                return False
                
        elif option_type == "PE":
            nifty_dir_pass = (20 < nifty_rsi < 45) and (close_price < ema20) and (close_price < vwap_price)
            opt_exec_pass = (opt_rsi > 60) and opt_volume_spike
            if not (nifty_dir_pass and atr_expanding and opt_exec_pass):
                return False
        ...
        if option_type == "PE":
             ...
             if fallback_signal is not None and fallback_signal.get("signal") == "BUY":
                  SafetyLogger.log_info("Option_strategy_core", f"Downtrend strategy fallback triggered: {fallback_signal.get('reason')}")
                  return True
...
def explain_trigger_failure(*args, **kwargs):
    ...
    try:
        ...
        if option_type == "CE":
            if nifty_rsi <= 70:
                return False, f"nifty_rsi_{nifty_rsi:.1f}_not_above_70", details
            ...
            if opt_rsi <= 60:
                return False, f"option_premium_rsi_{opt_rsi:.1f}_not_above_60", details
            if not opt_volume_spike:
                return False, "option_volume_spike_not_active", details
                
        elif option_type == "PE":
            if nifty_rsi >= 40:
                return False, f"nifty_rsi_{nifty_rsi:.1f}_not_below_40", details
            if close_price >= ema20:

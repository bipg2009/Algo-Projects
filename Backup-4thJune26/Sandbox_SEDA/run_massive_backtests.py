...
def copy_reports(phase_folder, chunk_name):
    ...
    dest_dir_local = os.path.join(WORKSPACE_DIR, "BackTesting", "Reports", "May-Runs", phase_folder, chunk_name)
...
def run_chunk(start_date, end_date):
    ...
    process = subprocess.Popen(cmd, cwd=WORKSPACE_DIR, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)

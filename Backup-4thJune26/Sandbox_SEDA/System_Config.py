...
TEST_MODE_INTERACTIVE: bool = True     # True = Route order to Test.py for manual confirm
...
BACKTEST_SLIPPAGE_PCT: float = 0.01  # 1% penalty on backtest entry price to model latency/premium jumps

# ---------------------------------------------------------
# DIRECTIONAL TOGGLES
# ---------------------------------------------------------
ENABLE_CE_TRADES: bool = True  # Disabled to cut noise during bearish macro trends
ENABLE_PE_TRADES: bool = True   # Focused entirely on downside setups

# ---------------------------------------------------------
# LOT SIZES — Centralised to prevent mismatch across modules
# Rule: NIFTY options default lot = 75 (per AGENTS.md)
# ---------------------------------------------------------
LOT_SIZES: dict = {
    "NIFTY": 75,
    "BANKNIFTY": 30,
...
# ---------------------------------------------------------
# RSI TRIGGERS & BANDS
# Why separated: Triggers control entry decisions (RiseFall_sub),
# bands control alert logging (logging_engine). Both must stay
# consistent; changing one without the other causes silent misses.
# ---------------------------------------------------------
CE_RSI_TRIGGER: float = 58.0
PE_RSI_TRIGGER: float = 40.0
CE_RSI_MIN: float = CE_RSI_TRIGGER   # Lower boundary of CE RSI band (same as trigger)
CE_RSI_MAX: float = 90.0             # Overbought ceiling for CE band alerts
PE_RSI_MIN: float = 10.0             # Oversold floor for PE band alerts
PE_RSI_MAX: float = PE_RSI_TRIGGER   # Upper boundary of PE RSI band (same as trigger)
BASE_RSI_POINTS: int = 10            # Points awarded per RSI alignment in scoring

# ---------------------------------------------------------
# RISK ENGINE
# ---------------------------------------------------------
GAP_RISK_THRESHOLD: int = 100          # Pts gap between prev close and today open to flag gap
...
# CHOP / ADX
# ---------------------------------------------------------
ADX_CHOP_THRESHOLD: float = 22.0      # Below this ADX the market is classified as chop/sideways
...
NORMAL_ITM_DISTANCE: int = 100
GAP_ITM_DISTANCE: int = 100

...
class Tradehull:    
	...
	def get_historical_data(self,tradingsymbol,exchange,days):			
		try:
			...
			instrument_exchange = {'NSE':"NSE",'BSE':"BSE",'NFO':'NSE','BFO':'BSE','MCX':'MCX','CUR':'NSE'}
			exchangeSegment = script_exchange[exchange]
			Symbol = self.instrument_df[((self.instrument_df['SEM_TRADING_SYMBOL']==tradingsymbol)|(self.instrument_df['SEM_CUSTOM_SYMBOL']==tradingsymbol))&(self.instrument_df['SEM_EXM_EXCH_ID']==instrument_exchange[exchange])].iloc[-1]['SEM_SMST_SECURITY_ID']
			...
			ohlc = self.Dhan.historical_daily_data(str(Symbol), exchangeSegment, instrument_type, from_date, to_date, expiry_code=int(expiry_code))

                ...
                try:
                    ...
                    if should_fetch:
                        ...
                        if opt_df is not None and not opt_df.empty and len(opt_df) >= 15:
                            ...
                            if vol_ema > 0:
                                opt_volume_spike = (discrete_v > (vol_ema * 2.0))
                except Exception as e:
                    print(f"[Core] Option premium analysis bypassed for {symbol}: {e}", flush=True)

        if option_type == "CE":
            nifty_dir_pass = (nifty_rsi > 60) and (close_price > ema20) and (close_price > vwap_price)
            opt_exec_pass = (opt_rsi > 70) and opt_volume_spike
...
def explain_trigger_failure(*args, **kwargs):
    ...
    try:
        ...
        if option_type == "CE":
            if nifty_rsi <= 60:
                return False, f"nifty_rsi_{nifty_rsi:.1f}_not_above_60", details
            ...
            if opt_rsi <= 70:
                return False, f"option_premium_rsi_{opt_rsi:.1f}_not_above_70", details

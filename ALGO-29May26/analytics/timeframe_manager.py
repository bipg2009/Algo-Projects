...
class CandleAggregator:
    ...
    def resample_1m_to_multi(df_1m: pd.DataFrame) -> dict:
        ...
        try:
            ...
            df_3m = df.resample('3min', closed='left', label='left').agg(agg_dict).dropna()
            df_5m = df.resample('5min', closed='left', label='left').agg(agg_dict).dropna()
            df_10m = df.resample('10min', closed='left', label='left').agg(agg_dict).dropna()
        except Exception as e:

...
def sync_excel_ltp(tsl, chain=None):
    ...
    try:
        ...
        # Keep REST diagnostics visible (throttled inside Tradehull) for faster live troubleshooting.
        rest_quotes = tsl.get_rest_quotes_for_watchlist(watchlist_rows, quiet=False) if watchlist_rows else {}
        ...
        for i in range(len(df)):
            ...
            if oi_row is None and not is_fno_symbol(sym):
                oi_row = [0, 0]
        ...
        if col_values:
            ...
            oi_rng = sheet.range(f"K2:L{end_row}")
            oi_rng.value = oi_values
            try:
                oi_rng.number_format = "0"
            except Exception:
                pass

...
import datetime
import pandas as pd
CE_RSI_MIN, CE_RSI_MAX = 58.0, 70.0
PE_RSI_MIN, PE_RSI_MAX = 30.0, 42.0
...
def print_heartbeat(df_1m: pd.DataFrame, chain: dict, target_type: str, valid_options: list, trigger_found: bool, pcr_val: float) -> None:
    ...
    try:
        ...
        state_str = (
            f"   |-- [SEDA] 10m:{regime.market_regime} | "
            f"5m:{structure.trend_structure} | 3m:{confirmation.vwap_state} | "
            f"1m:{execution.momentum_velocity}"
        )

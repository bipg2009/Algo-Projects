...
import threading
def log_execution_to_excel_ledger(*args, **kwargs):
    threading.Thread(target=_sync_log_ledger, args=args, kwargs=kwargs, daemon=True).start()
import SafetyLogger
    ...
    while True:
        try:
            ...
            if action == "EXIT":
                ...
                for _ in range(6000): # Wait up to 600s check at 100ms
                    try:

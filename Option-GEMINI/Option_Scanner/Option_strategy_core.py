...
def validate_oi_barrier_distance(nifty_spot, option_type, option_chain_df, min_distance=60):
    try:
        ...
        if option_type == "CE":
            calls_above = option_chain_df[
                (option_chain_df['option_type'] == 'CE') & 
                (option_chain_df['strike'] > nifty_spot)
            ]
            if calls_above.empty:
                return True, 0
                
            max_oi_idx = calls_above['oi'].idxmax()
            highest_oi_strike = calls_above.loc[max_oi_idx, 'strike']
            
            distance = highest_oi_strike - nifty_spot
            return (distance >= min_distance), highest_oi_strike

        if option_type == "PE":
            puts_below = option_chain_df[
            ...
            max_oi_idx = puts_below['oi'].idxmax()
            highest_oi_strike = puts_below.loc[max_oi_idx, 'strike']
...
def validate_volume_support(option_type, option_chain_df, min_ratio=1.2):
    try:
        ...
        total_ce_vol = option_chain_df[option_chain_df['option_type'] == 'CE'].get('volume', pd.Series([0])).sum()
        total_pe_vol = option_chain_df[option_chain_df['option_type'] == 'PE'].get('volume', pd.Series([0])).sum()
...
def build_and_score_contract(opt_row, option_type, df_1m, pcr_value=1.0):
    try:
        ...
        if df_1m is None or df_1m.empty:
            return 0, {}
        work = Indicators.add_vwap(df_1m.copy())

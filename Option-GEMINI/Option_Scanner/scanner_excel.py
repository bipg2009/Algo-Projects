...
def play_alert_sound(alert_type="buy"):
    """
    Play a Windows notification sound (async, non-blocking).

    cred.env (optional):
      ALERT_SOUND_ENABLED=1
      ALERT_SOUND_BUY=1
      ALERT_SOUND_SELL=1
      ALERT_SOUND_BUY_WAV=C:\\Windows\\Media\\Windows Notify System Generic.wav
      ALERT_SOUND_SELL_WAV=C:\\Windows\\Media\\Windows Notify Calendar.wav
      ALERT_SOUND_WAV=...  (fallback for both if type-specific not set)
    """
    if os.name != "nt":
        return
    try:
    ...
    if not _env_on("ALERT_SOUND_ENABLED", "1"):
        return
    kind = str(alert_type).strip().lower()
    if kind == "buy" and not _env_on("ALERT_SOUND_BUY", "1"):
        return
    if kind == "sell" and not _env_on("ALERT_SOUND_SELL", "1"):
        return

    wav = os.getenv(f"ALERT_SOUND_{kind.upper()}_WAV", "").strip()
    if not wav:
        wav = os.getenv("ALERT_SOUND_WAV", "").strip()
    try:
        import winsound

        flags = winsound.SND_ASYNC
        if wav and os.path.isfile(wav):
            winsound.PlaySound(wav, winsound.SND_FILENAME | flags)
            return
        alias = "SystemAsterisk" if kind == "buy" else "SystemHand"
        ...
        try:
            ...
            winsound.MessageBeep(
                winsound.MB_ICONASTERISK if kind == "buy" else winsound.MB_ICONHAND
            )
        except Exception:
            pass
...
def print_alert_sound_help():
    if os.name != "nt":
        print("[i] Alert sounds: Windows only (not configured on this OS).", flush=True)
        return
    print("[i] Alert sounds: BUY=SystemAsterisk, SELL=SystemHand (set ALERT_SOUND_* in cred.env).", flush=True)

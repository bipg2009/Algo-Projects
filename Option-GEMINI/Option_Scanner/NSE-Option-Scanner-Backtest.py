...
def build_chain_at_bar(legs, bar_opt_idx, ref_date, underlying):
    ...
    if not options:
        return None
    spot = float(legs[0]["series"]["spot"][bar_opt_idx])
...
def _book_row(test_date, bar_time, trade_id, event, pick, price, sl, rsi="", oi=""):
    ...
    return {
        ...
        "rsi": rsi,
        "oi": oi,
        "outcome": bh.trade_outcome(price, sl, is_sell),
    }
...
def simulate_trade(pick, entry_1m_idx, nifty_1m, legs, test_date):
    ...
    entry_ltp = opt["ltp"]
    levels = bh.entry_levels(entry_ltp)
    position = {
        "symbol": pick["display_symbol"],
        "option_type": pick["option_type"],
        "entry_ltp": entry_ltp,
    ...
    trade_book.append(_book_row(
        ...
        pick, entry_ltp, levels["sl"], entry_rsi, entry_oi,
    ))
    ...
    sell_rows = [r for r in trade_book if r.get("event") == "SELL"]
    last_outcome = sell_rows[-1]["outcome"] if sell_rows else ""
...
def run_backtest(test_date, expiry_code, results_dir=None):
    sc = load_scanner_module()
    print(f"\nBACKTEST | {test_date} | 1m SETUP + 1m TRIGGER | expiry_code={expiry_code}\n")
    ...
    print("--- TRADES (1m trigger via Option_strategy_core) ---")

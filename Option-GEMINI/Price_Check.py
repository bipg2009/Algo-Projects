...
def get_live_client():
    if CRED_ENV.is_file():
        try:
            ...
            SafetyLogger.log_error_with_context("Price_Check", "load_cred_env", e)
    ...
    try:
        ...
        SafetyLogger.log_error_with_context("Price_Check", "get_live_client", e)
        return None
        ...
        try:
            ...
            if df_1m is not None and not df_1m.empty and len(df_1m) >= 15:
                try:
                    ...
                    current_rsi = monitor_engine.execute_hypercare_monitoring.__globals__["Indicators"].calculate_rsi(df_1m)
                except Exception as e:
            ...
            try:
                action, reason, updated_sl = monitor_engine.execute_hypercare_monitoring(position, df_1m, opt_row, current_ltp)
            except Exception as e:
                SafetyLogger.log_error_with_context(
            ...
            if action == "TRAIL":
                ...
                print(f"[TRAIL ALERT] Stop Loss trailed higher to Rs {updated_sl}", flush=True)
            ...
            if action == "EXIT":
                        ...
                        if completed_qty > 0:
                            try:
                                log_execution_to_excel_ledger(
                                    ...
                                    price=current_ltp,
                                    margin_used=position.get("margin_used", 0) * (completed_qty/(qty or 1))
                                )
                            except:
                                pass
                        else:
                            ...
                            if completed_qty < pending_qty:
                                ...
                                try:
                                    log_execution_to_excel_ledger(
                                        ...
                                        quantity=pending_qty,
                                        price=current_ltp,
                                        margin_used=position.get("margin_used", 0) * (pending_qty / (qty or 1))
                                    )
                                except:
                                    pass
                ...
                # Write state file for MainEngine to review
                sell_state_file = Path(__file__).resolve().parent / "exit_signal.json"
                try:
                    with open(sell_state_file, "w", encoding="utf-8") as f:
                        json.dump({
                            "order_id": order_id,
                            "symbol": option_symbol,
                            "qty": qty,
                    ...
                    print("[+] exit_signal.json written. Awaiting MainEngine.py to verify and confirm...", flush=True)
                except Exception as e:
                    SafetyLogger.log_error_with_context("Price_Check", "write_exit_signal_json", e)

                # Wait for MainEngine to confirm and delete the json file
                verified_by_main_engine = False
                for _ in range(600): # Wait up to 600s
                    try:
                        if not sell_state_file.exists():
                            verified_by_main_engine = True
                            break
                    except Exception:
                        pass
                    time.sleep(1.0)
                
                if verified_by_main_engine:
                    print(f"[\033[92mCONFIRMED\033[0m] SELL Order successfully verified by Main Engine!", flush=True)
                    print("SELL order fully executed and confirmed.", flush=True)
                else:
                    print(f"[\033[91mWARNING\033[0m] Timed out waiting for Main Engine verification. Order might be stuck.", flush=True)
                ...
                print("MainEngine has instructed Price_Check & Monitor_Engine to sleep. Exiting.", flush=True)
                time.sleep(5.0)  # Short pause so you can read the final terminal status block before it auto-closes
                break

        except Exception as e:
            SafetyLogger.log_error_with_context("Price_Check", "price_check_master_loop", e)
        
        # Hibernate exactly 30 seconds before printing the next update block
        time.sleep(30.0)

...
def push_ticker_to_dashboard(chain_data):
    ...
    ticker_data = sorted(ticker_data, key=lambda x: abs(x.get("oi_change_pct", 0)), reverse=True)
...
def build_and_evaluate_candidate(candidate, target_type, df_1m, pcr_val, chain, nifty_spot, previous_close, today_open, st_color):
    ...
    if score >= 70:
        ...
        if not passed and score >= core.STRONG_BUY_THRESHOLD:
            log_dashboard(f"[scan] {candidate.get('symbol')} score={score} reject: {reason}")
    ...
    if score >= core.STRONG_BUY_THRESHOLD:
        ...
        if core.detect_trigger_1m(df_1m, target_type, candidate, nifty_spot, previous_close, today_open, pcr_val, chain_df):
            log_dashboard(f"🔥 SIGNAL VERIFIED! Score: {score} on {candidate['symbol']}. Handing off to Main Engine.", level="success")
    ...
    if cm_signal and cm_signal.get("option_type") == target_type:
        log_dashboard(f"⚖️ CHOP-MODE SIGNAL! Mean reversion on {cm_signal['symbol']}.", level="success")
        print(f"\n⚖️ CHOP-MODE SIGNAL! Mean reversion on {cm_signal['symbol']}.", flush=True)
        play_alert_sound("buy")
        set_shared_state("MUTE")
        dashboard_queue.put({"type": "SCANNER_STATE", "payload": {"status": get_shared_state(), "mute_seconds_left": 0}})
        ...
        signal_data["score"] = int(score)
        signal_queue.put(signal_data)
        dashboard_queue.put({"type": "NEW_SIGNAL", "payload": {"symbol": cm_signal["symbol"], "type": target_type, "action": "BUY (Chop)", "time": datetime.datetime.now().strftime("%H:%M:%S"), "strategy": "Chop_Mode"}})
        return True

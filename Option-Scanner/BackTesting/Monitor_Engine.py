...
_STALE_HOLD_MIN: int = 6  # Minutes before time-based exit check (was 12)
_STALE_HOLD_PTS: float = 2.0  # Min profit required to hold past time limit (was 0.0)


# ── Exit checks (each ≤30 LOC) ──────────────────────────


def _check_target_hit(current_ltp: float, target: float, sl: float) -> tuple:
...
def _check_trailing_stop(
    ...
    """Implements Phase-based Trailing SL with Time Checks."""
    ...
    elapsed_min = (
        pd.to_datetime(current_time) - pd.to_datetime(entry_time)
    ).total_seconds() / 60.0

    # Phase 1: 0-2 mins
    if elapsed_min < 2.0:
        return None  # Strict 15-point structural SL (handled in _check_stop_loss)

    # EXACTLY at 2 mins (or first tick >= 2.0)
    # The requirement: "At 2 mins: If LTP < Buying Price - 1% Exit"
    # To prevent immediate exit on a later tick, we enforce this check only if SL hasn't been upgraded to Entry - 1% yet.
    cost_minus_1pct = round(position.entry_ltp * 0.75, 2)
    if elapsed_min >= 2.0 and position.sl < cost_minus_1pct:
        if current_ltp < cost_minus_1pct:
            print(
                f"[{current_time}] EXIT: 2_min_profit_check_failed at LTP {current_ltp}",
                flush=True,
            )
            return ("EXIT", "2_min_profit_check_failed", current_ltp)
        else:
            # Upgrade SL immediately to Cost - 1%
            return ("TRAIL", "2_min_cold_protection_end", cost_minus_1pct)

    # At 6 mins momentum check:
    # "If LTP < Entry + ₹2.50 Exit"
    cost_plus_250 = round(position.entry_ltp + 2.50, 2)
    if elapsed_min >= 6.0 and position.sl < cost_plus_250:
        if current_ltp < cost_plus_250:
            print(
                f"[{current_time}] EXIT: 6_min_momentum_failed at LTP {current_ltp}",
                flush=True,
            )
            return ("EXIT", "6_min_momentum_failed", current_ltp)
        else:
            # Floor SL to Cost + 2.50
            return ("TRAIL", "6_min_momentum_passed", max(position.sl, cost_plus_250))

    # Determine trailing distance based on trade age
    # Restored to original 10-point logic which gave the trade perfect breathing room
    trail_dist = 5.0 if elapsed_min >= 12.0 else 10.0
    dynamic_sl = round(peak - trail_dist, 2)

    # We never let the new SL drop below the strictly enforced phase minimums
    phase_min_sl = cost_plus_250 if elapsed_min >= 6.0 else cost_minus_1pct
    new_sl = max(dynamic_sl, phase_min_sl)

    # If trailing SL is hit
    if current_ltp <= new_sl:
        reason = "dynamic_trail_hit"
        print(
            f"[{current_time}] EXIT: {reason} at LTP {current_ltp} (SL was {new_sl})",
            flush=True,
        )
        return ("EXIT", reason, new_sl)

    if new_sl > position.sl:
        reason = f"{int(trail_dist)}_pt_dynamic_trail"
        print(f"[{current_time}] Phase 2: SL moved to {new_sl} ({reason})", flush=True)
...
def _check_supertrend_reversal(
    ...
    if df_5m is not None and not df_5m.empty and "st_color" in df_5m.columns:
        ...
        if (option_type == "CE" and st_5m == "GREEN") or (
            option_type == "PE" and st_5m == "RED"
        ):
            return None
...
def _check_oi_unwinding(entry_oi: int, opt_row: dict) -> tuple:
                ...
                if df_1m is not None and not df_1m.empty and len(df_1m) >= 3:
                    ...
                    if c_prev and c_curr and is_65_pct_formed:
                        ...
                        if new_sl > position.sl:
                            return (
                                "TRAIL",
                                f"{target_minutes}m_momentum_st_cascade",
                                new_sl,
                            )
    return None
...
def execute_hypercare_monitoring(
    ...
    try:
        ...
        current_time = (
            df_1m.iloc[-1]["start_Time"]
            if not df_1m.empty and "start_Time" in df_1m.columns
            else None
        )

        result = _check_trailing_stop(position, current_ltp, current_time, df_1m)
        if result:
            return result

        result = _check_supertrend_reversal(
            df_1m, option_type, current_ltp, position, is_backtest, df_5m
        )
        if result:
            return result
        ...
        result = _check_time_decay_cascade(
            position, df_1m, df_5m, current_ltp, current_time, option_type, is_backtest
        )
        if result:
            return result

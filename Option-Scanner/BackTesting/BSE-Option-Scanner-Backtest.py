...
print("BSE SENSEX Option Scanner BACKTEST — starting...", flush=True)

TEST_DATE = "2026-05-22"
EXPIRY_CODE = 1
API_PAUSE_SEC = 1.2
SENSEX_SECURITY_ID = 51
OPTION_BAR_TF = 5  # Dhan option history bars (LTP only; SENSEX setup/trigger use 1m)
...
def fetch_sensex_bars(tsl, test_date, interval=1):
    cache = cache_path(f"sensex_{interval}m_{test_date}.json")
    ...
    resp = tsl.Dhan.intraday_minute_data(
        str(SENSEX_SECURITY_ID), "IDX_I", "INDEX", test_date, next_day, interval=interval
    )
    data = resp.get("data") or {}
    if not data.get("timestamp"):
        raise RuntimeError(f"No SENSEX {interval}m data for {test_date}")
...
def fetch_one_option_leg(tsl, strike_offset, opt_type, test_date, expiry_code):
    ...
    resp = tsl.Dhan.expired_options_data(
        str(SENSEX_SECURITY_ID), "BSE_FNO", "OPTIDX", "WEEK", expiry_code,
...
def fetch_option_legs(tsl, test_date, expiry_code):
    cache = cache_path(f"sensex_options_{OPTION_BAR_TF}m_{test_date}_ec{expiry_code}.json")
    if os.path.isfile(cache):
...
def build_chain_at_bar(legs, bar_opt_idx, ref_date):
    options = []
    for item in legs:
        s = item["series"]
        ...
        if volume <= 0 and ltp <= 0:
            continue
        label = format_option_label("SENSEX", strike, item["option_type"], ref_date)
        options.append({
...
def simulate_trade(pick, entry_1m_idx, sensex_1m, legs, test_date, sensex_5m):
    import System_Config
    
    # N+1 Execution: Force entry on the NEXT candle to simulate realistic latency/open fill
    if entry_1m_idx + 1 < len(sensex_1m):
        entry_1m_idx += 1
    ...
    current_atr = sensex_1m.iloc[entry_1m_idx].get("ATR", 15.0)
    levels = bh.entry_levels(entry_ltp, current_atr)
    entry_spot = float(sensex_1m.iloc[entry_1m_idx]["close"])
    ...
    entry_time = sensex_1m.iloc[entry_1m_idx]["start_Time"]
    entry_rsi = core.calculate_rsi(bh.prepare_nifty_1m(sensex_1m.iloc[: entry_1m_idx + 1]))
    ...
    for i in range(entry_1m_idx + 1, len(sensex_1m)):
        row_1m = sensex_1m.iloc[i]
        ...
        if bh.bar_time_ist(bar_time) >= bh.MARKET_EXIT_TIME:
            ...
            exit_1m_idx = i
            exit_rsi = core.calculate_rsi(bh.prepare_nifty_1m(sensex_1m.iloc[: i + 1]))
        ...
        df_slice = bh.prepare_nifty_1m(sensex_1m.iloc[: i + 1])
        ...
        pos_obj = TradePosition(
            symbol="SENSEX", option_type=position["option_type"], qty=10,
        ...
        if sensex_5m is not None and not sensex_5m.empty:
            df_5m_slice = sensex_5m[sensex_5m["start_Time"] <= bar_time].copy()
    ...
    if exit_reason == "EOD":
        row_1m = sensex_1m.iloc[exit_1m_idx]
        trade_book.append(_book_row(
            test_date, row_1m["start_Time"], trade_id, "SELL",
            pick, exit_ltp, position["sl"],
            core.calculate_rsi(bh.prepare_nifty_1m(sensex_1m.iloc[: exit_1m_idx + 1])),
            get_option_snapshot(legs, pick, option_bar_idx(exit_1m_idx)).get("oi", ""),
            entry_price=position["entry_ltp"]
        ))

    exit_time = sensex_1m.iloc[exit_1m_idx]["start_Time"]
...
def run_backtest(test_date, expiry_code, results_dir=None, candle_mode="NORMAL"):
    ...
    print("Loading SENSEX 1m and 5m...")
    sensex_1m = fetch_sensex_bars(sc.tsl, test_date, 1)
    sensex_5m = fetch_sensex_bars(sc.tsl, test_date, 5)

    test_day = pd.to_datetime(test_date).date()
    sensex_1m = sensex_1m[pd.to_datetime(sensex_1m["start_Time"]).dt.date == test_day].reset_index(drop=True)

    # Apply Candle Transformations
    import candle_transformer
    if candle_mode == "HEIKIN_ASHI":
        print("  Applying Heikin-Ashi candle transformation to SENSEX spot...")
        sensex_1m = candle_transformer.to_heikin_ashi(sensex_1m)
    elif candle_mode == "VOLUME":
        print("  Applying Volume candle transformation to SENSEX spot...")
        sensex_1m = candle_transformer.to_volume_candles(sensex_1m)

    print(f"  {len(sensex_1m)} x 1m candles")

    print(f"Loading options ({OPTION_BAR_TF}m LTP bars)...")
    legs = align_legs(fetch_option_legs(sc.tsl, test_date, expiry_code), len(sensex_1m) // OPTION_BAR_TF + 1)
    ...
    session_indices = [
        i
        for i in range(len(sensex_1m))
        if bh.is_within_session(sensex_1m.iloc[i]["start_Time"])
    ]
    print(
    ...
    prev_close = float(sensex_1m.iloc[0]["close"])
    today_open = float(sensex_1m.iloc[session_indices[0]]["open"]) if session_indices else prev_close
    ...
    for i in session_indices:
        ...
        bar_time = sensex_1m.iloc[i]["start_Time"]
        chain = build_chain_at_bar(legs, option_bar_idx(i), test_date)
        df_slice = bh.prepare_nifty_1m(sensex_1m.iloc[: i + 1]) if len(sensex_1m.iloc[: i + 1]) >= 30 else None
        ...
        trade = simulate_trade(pick, i, sensex_1m, legs, test_date, sensex_5m)
        ...
        in_trade_until = i + len(trade["monitor_log"]) + 10
...
if __name__ == "__main__":
    ...
    print("For full multi-day testing across all modes, please run: multi_day_SENSEX-test.py")
    print("!" * 80 + "\n")
    
    raw_date = sys.argv[1] if len(sys.argv) > 1 else TEST_DATE
    try:
        # Gracefully handle DD-MM-YYYY or YYYY-MM-DD inputs from CLI
        parsed_date = pd.to_datetime(raw_date, format="%d-%m-%Y" if "-" in raw_date and len(raw_date.split("-")[0]) == 2 else None).strftime("%Y-%m-%d")
    except:
        parsed_date = pd.to_datetime(raw_date).strftime("%Y-%m-%d")
        
    try:
        run_backtest(parsed_date, EXPIRY_CODE)

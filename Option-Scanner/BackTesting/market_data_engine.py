...
def calculate_live_pcr(chain: dict) -> float:
    ...
    # Instantly cache the aggregated data directly into the chain object for global O(1) access
    chain["total_ce_oi"] = total_call_oi
    chain["total_pe_oi"] = total_put_oi
    
    total_call_vol = sum(int(o.get("volume", 0)) for o in options if isinstance(o, dict) and o.get("option_type") == "CE")
    total_put_vol = sum(int(o.get("volume", 0)) for o in options if isinstance(o, dict) and o.get("option_type") == "PE")
    chain["total_ce_vol"] = total_call_vol
    chain["total_pe_vol"] = total_put_vol
...
def _fetch_index_chain(tsl, underlying: str, strike_range: int, expiry_ref: str, cache_attr: str, expiry_attr: str, fail_log_attr: str):
    ...
    if chain and isinstance(chain, dict) and chain.get("options"):
        raw_chain_options = list(chain.get("options", []))
        chain = filter_chain_strike_window(dict(chain))
        chain["_raw_options"] = raw_chain_options

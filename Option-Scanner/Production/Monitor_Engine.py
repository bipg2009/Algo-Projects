...
def _check_trailing_stop(
    ...
    """Implements Antigravity breakeven trigger and 15-point trailing SL."""
    peak: float = max(position.peak_price, current_ltp)
    position.peak_price = peak

    # Step 1: Trail to breakeven (+1.0 pt buffer) when peak >= entry_ltp + 15.0
    if not getattr(position, "breakeven_triggered", False) and peak >= (position.entry_ltp + 15.0):
        new_sl = round(position.entry_ltp + 1.0, 2)
        if new_sl > position.sl:
            return ("TRAIL", "breakeven_triggered", new_sl)

    # Step 2: Trailing SL of 15 premium points once peak exceeds entry + 20
    if peak >= (position.entry_ltp + 20.0):
        new_sl = round(peak - 15.0, 2)
        if new_sl > position.sl:
            return ("TRAIL", "trailing_stop_raised", new_sl)

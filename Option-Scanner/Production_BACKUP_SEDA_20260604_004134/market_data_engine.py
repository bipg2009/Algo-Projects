...
def get_nifty_1m_cached(tsl) -> pd.DataFrame:
    ...
    global _intraday_fail_last_log
    now = time.time()
    ...
    if df is None:
        ...
        # Log underlying Dhan status/remarks occasionally (helps debug silent None)
        if now - _intraday_fail_last_log > 30:
            _intraday_fail_last_log = now
            try:
                resp = getattr(tsl, "Dhan", None).get_fund_limits() if getattr(tsl, "Dhan", None) else None
                if isinstance(resp, dict):
                    print(f"[DEBUG] dh_fund_limits status={resp.get('status')} remarks={resp.get('remarks')}", flush=True)
            except Exception as e:
                print(f"[DEBUG] dh_fund_limits error: {type(e).__name__}: {e}", flush=True)

            # Try raw intraday response (even if wrapper returns None)
            try:
                import datetime
                to_date = datetime.datetime.now().strftime("%Y-%m-%d")
                from_date = (datetime.datetime.now() - datetime.timedelta(days=7)).strftime("%Y-%m-%d")
                raw = getattr(tsl, "Dhan", None).intraday_minute_data("13", getattr(tsl, "Dhan", None).INDEX, "INDEX", from_date, to_date) if getattr(tsl, "Dhan", None) else None
                if isinstance(raw, dict):
                    print(f"[DEBUG] dh_intraday status={raw.get('status')} remarks={raw.get('remarks')} data_type={type(raw.get('data')).__name__}", flush=True)
            except Exception as e:
                print(f"[DEBUG] dh_intraday error: {type(e).__name__}: {e}", flush=True)
    elif df.empty:

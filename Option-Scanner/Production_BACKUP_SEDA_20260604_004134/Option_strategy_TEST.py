...
from System_Config import (
    STRONG_BUY_THRESHOLD, TACTICAL_LOOKBACK, EXHAUSTION_WINDOW,
...
def validate_oi_support(option_type, option_chain_df, min_ratio=1.2):
    """
    Validates if Open Interest favors the directional trade.
    For CE (Bullish): PE OI (Support) > CE OI (Resistance) * min_ratio
    For PE (Bearish): CE OI (Resistance) > PE OI (Support) * min_ratio
    """
    try:
        if option_chain_df is None or option_chain_df.empty:
            return True
        ...
        if 'oi' in option_chain_df.columns:
            ois = option_chain_df['oi'].values
        else:
            ois = np.zeros(len(option_chain_df))
            
        total_ce_oi = np.sum(ois[opt_types == 'CE'])
        total_pe_oi = np.sum(ois[opt_types == 'PE'])
        
        if total_ce_oi == 0 or total_pe_oi == 0:
            return False
            
        if option_type == "CE":
            return total_pe_oi > (total_ce_oi * min_ratio)
            
        if option_type == "PE":
            return total_ce_oi > (total_pe_oi * min_ratio)
            
        return False
    except Exception as e:
        SafetyLogger.log_error_with_context(
            "Option_strategy_core", "validate_oi_support", e,
            {"option_type": option_type, "df_len": len(option_chain_df) if option_chain_df is not None else 0}
        )
        return True # Fallback to True
...
def build_score(opt_row, option_type, df_1m, pcr_value=1.0, gap_mode=False):
    try:
        ...
        oi_change_pct = opt_row.get("oi_change_pct", 0)
        ...
        # -----------------------------------------
        # OI BUILDUP (Option Buyer Perspective)
        # Increasing OI on a strike = institutional writing (resistance/support)
        # Decreasing OI on a strike = unwinding/covering (breakout)
        # -----------------------------------------
        if oi_change_pct > 5:
            score -= 15  # Fresh writing on the strike we want to buy -> BAD
        elif oi_change_pct < -2:
            score += 15  # Unwinding/Covering on the strike we want to buy -> GOOD
        ...
        score = max(0, score) # Upper bound removed to track ultra-high conviction trends
        return score
    except Exception as e:
        SafetyLogger.log_error_with_context(
...
def detect_trigger_1m(*args, **kwargs):
    try:
        ...
        has_volume_support = validate_volume_support(option_type, option_chain_df, min_ratio=0.8)
        if not has_volume_support:
            return False

        import OrderFlow_Engine
        orderflow_sentiment = OrderFlow_Engine.get_engine().get_orderflow_sentiment()
        if option_type == "CE" and orderflow_sentiment == "BEARISH":
            # Print for logging
            print(f"[Core] Trade Blocked for {opt_row.get('symbol')}: Adverse Institutional Order Flow (BEARISH)", flush=True)
            return False
        if option_type == "PE" and orderflow_sentiment == "BULLISH":
            print(f"[Core] Trade Blocked for {opt_row.get('symbol')}: Adverse Institutional Order Flow (BULLISH)", flush=True)
            return False

        # OI Support Check
        has_oi_support = validate_oi_support(option_type, option_chain_df, min_ratio=0.8)
        ...
        prev_nifty_rsi = rsi_series.iloc[-3] if len(rsi_series) >= 3 else nifty_rsi
        row = df_1m.iloc[-2]
        close_price = row["close"]
        vwap_price = row["VWAP"]
        ema20 = row["EMA20"]
        ...
        if option_type == "CE":
             if rise_fall.evaluate_ce_trigger(current_rsi, tactical_slice, ce_exhaustion_slice, st_color):
                  if isinstance(opt_row, dict):
                      opt_row["strategy_suffix"] = "General Buy / Sell"
                  return True
             ...
             if fallback_signal is not None and fallback_signal.get("signal") == "BUY":
                  ...
                  if isinstance(opt_row, dict):
                      opt_row["strategy_suffix"] = "Uptrend Buy"
                  return True
        ...
        if option_type == "PE":
             ...
             if fallback_signal is not None and fallback_signal.get("signal") == "BUY":
                  ...
                  if isinstance(opt_row, dict):
                      opt_row["strategy_suffix"] = "Downtrend Buy"
                  return True
...
def explain_trigger_failure(*args, **kwargs):
    ...
    try:
        ...
        # OI checking
        has_oi_support = validate_oi_support(option_type, option_chain_df, min_ratio=1.2)
        details["oi_support_passed"] = has_oi_support
        if not has_oi_support:
            return False, "insufficient_directional_oi_ratio", details
        ...
        if option_type == "CE":
            ...
            if not is_fallback:
                ...
                bullish_exhaustion = (nifty_rsi > 85) and (nifty_rsi < prev_nifty_rsi)
                if nifty_rsi < System_Config.CE_RSI_TRIGGER:
                    return False, f"nifty_rsi_{nifty_rsi:.1f}_below_{System_Config.CE_RSI_TRIGGER}", details
                if bullish_exhaustion:
                    return False, f"nifty_rsi_{nifty_rsi:.1f}_exhausted_from_{prev_nifty_rsi:.1f}", details
            ...
            if not is_fallback:
                import System_Config
                bearish_exhaustion = (nifty_rsi < 15) and (nifty_rsi > prev_nifty_rsi)
                if nifty_rsi > System_Config.PE_RSI_TRIGGER:
                    return False, f"nifty_rsi_{nifty_rsi:.1f}_above_{System_Config.PE_RSI_TRIGGER}", details
